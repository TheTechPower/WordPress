...
class aakanksha_Contact_Info extends WP_Widget {

    function aakanksha_contact_info() {
		$widget_ops = array('classname' => 'aakanksha_contact_info_widget', 'description' => __( 'Display your contact info', 'aakanksha') );
        parent::__construct(false, $name = __('aakanksha: Contact info', 'aakanksha'), $widget_ops);
		$this->alt_option_name = 'aakanksha_contact_info';	
    }
	...
	<p><label for="<?php echo $this->get_field_id( 'address' ); ?>"><?php _e( 'Enter your address', 'aakanksha' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'phone' ); ?>"><?php _e( 'Enter your phone number', 'aakanksha' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e( 'Enter your email address', 'aakanksha' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['aakanksha_contact_info']) )
			delete_option('aakanksha_contact_info');		  
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'aakanksha_contact_info', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'aakanksha_contact_info', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
