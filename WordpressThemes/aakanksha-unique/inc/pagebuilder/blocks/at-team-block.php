...
// CLEAR BLOCK
if( ! class_exists( 'as_Team_Block' ) ) :

class as_Team_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT Team', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('as_Team_Block', $block_options);
	}
 	...
 	function form($instance) {
        ...
        <p class="description">
			<label for="<?php echo $this->get_field_id('slide_speed') ?>">
                ...
                <?php echo as_field_input('slide_speed', $block_id, $slide_speed, 'min', 'number') ?> ms
            </label>&nbsp;-&nbsp;
            <label for="<?php echo $this->get_field_id('pagination_speed') ?>">
                Pagination speed in milliseconds
                <?php echo as_field_input('pagination_speed', $block_id, $pagination_speed, 'min', 'number') ?> ms
            </label>
		</p>
        <p class="description">
            <label for="<?php echo $this->get_field_id('auto_play') ?>">
                ...
                <?php echo as_field_select('auto_play', $block_id, $auto_play_type , $auto_play) ?>
            </label>
        </p>
        <p class="description">
            <label for="<?php echo $this->get_field_id('navigation') ?>">
                Display "next" and "prev" buttons.<br/>
                <?php echo as_field_select('navigation', $block_id, $navigation_type , $navigation) ?>
            </label>
        </p>
        <p class="description">
            <label for="<?php echo $this->get_field_id('pagination') ?>">
                Show pagination.<br/>
                <?php echo as_field_select('pagination', $block_id, $pagination_type , $pagination) ?>
            </label>
        </p>
        <p class="description">
            <label for="<?php echo $this->get_field_id('pagination_numbers') ?>">
                Show numbers inside pagination buttons<br/>
                <?php echo as_field_select('pagination_numbers', $block_id, $pagination_numbers_type , $pagination_numbers) ?>
            </label>
        </p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('items') ?>">
				...
				<?php echo as_field_input('items', $block_id, $items, 'min', 'number') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('items_desktop') ?>">
				...
				<?php echo as_field_input('items_desktop', $block_id, $items_desktop, 'min', 'number') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('items_desktop_small') ?>">
				Items Desktop Small [ window <= 979px ] ( *Note : As above.)
				<?php echo as_field_input('items_desktop_small', $block_id, $items_desktop_small, 'min', 'number') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('items_tablet') ?>">
				Items Tablet [ window <= 768px]	( *Note : As above.)
				<?php echo as_field_input('items_tablet', $block_id, $items_tablet, 'min', 'number') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('items_mobile') ?>">
				Items Mobile [ window <= 479px ] ( *Note : As above.)
				<?php echo as_field_input('items_mobile', $block_id, $items_mobile, 'min', 'number') ?>
			</label>
		</p>
		<?php
	}
	...
	function block($instance) {
		...
		?>
			...
			<div class="at-team-post item <?php echo $animation_effect ?>" <?php echo $duration_effect ?>>
                ...
                <div class="team-info">
                	...
                	<span class="position"><?php echo get_post_meta( $post->ID, 'as_team_position', true ); ?></span>
                    <div class="social-share team-social">
                        <ul class="social">
                            <?php if(get_post_meta( $post->ID, 'as_team_fb', true )){ ?>
                            <li><a target="_blank" href="<?php echo get_post_meta( $post->ID, 'as_team_fb', true ); ?>"><i class="fa fa-facebook"></i></a></li>
                            <?php } ?>
                            <?php if(get_post_meta( $post->ID, 'as_team_tw', true )){ ?>
                            <li><a target="_blank" href="<?php echo get_post_meta( $post->ID, 'as_team_tw', true ); ?>"><i class="fa fa-twitter"></i></a></li>
                            <?php } ?>
                            <?php if(get_post_meta( $post->ID, 'as_team_db', true )){ ?>
                            <li><a target="_blank" href="<?php echo get_post_meta( $post->ID, 'as_team_db', true ); ?>"><i class="fa fa-dribbble"></i></a></li>
                            <?php } ?>
                            <?php if(get_post_meta( $post->ID, 'as_team_gplus', true )){ ?>
                            <li><a target="_blank" href="<?php echo get_post_meta( $post->ID, 'as_team_gplus', true ); ?>"><i class="fa fa-google-plus"></i></a></li>
                            <?php } ?>
                        </ul>
                    </div>
                </div>
...
as_register_block( 'as_Team_Block' );
endif;