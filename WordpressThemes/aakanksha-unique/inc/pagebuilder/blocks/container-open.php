...
if( ! class_exists( 'as_Open_Container_Block') ) :

class as_Open_Container_Block extends as_Block {
	...
	function __construct() {
		...
		parent::__construct( 'as_Open_Container_Block', $block_options );
	}
	...
	function form( $instance ){
        ...
        <p class="description">
			<label for="<?php echo $this->get_field_id( 'padding_top' ) ?>">
				<?php _e( 'Padding top', 'aakanksha');?>
				<?php echo as_field_input( 'padding_top', $block_id, $padding_top, $size = 'min', $type = 'number' ) ?>px
            </label>
            &nbsp;&nbsp;-&nbsp;&nbsp;
			<label for="<?php echo $this->get_field_id( 'padding_bottom' ) ?>">
				<?php _e( 'Padding Bottom', 'aakanksha');?>
				<?php echo as_field_input( 'padding_bottom', $block_id, $padding_bottom, $size = 'min', $type = 'number' ) ?>px
            </label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('text_color') ?>">
				<?php _e( 'Text color', 'aakanksha');?>
				<?php echo as_field_select('text_color', $block_id, $text_colors, $text_color) ?>
			</label>
		</p>
		<p class="description">
			<label for="<?php echo $this->get_field_id('image') ?>">
				<?php _e( 'Background Image', 'aakanksha');?>
				<?php echo as_field_upload('image', $block_id, $image, $media_type = 'image') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('overlay_color') ?>">
				<?php _e( 'Overlay Color and transparency (Ex : rgba(0,0,0,.7);)', 'aakanksha');?>
                <?php echo as_field_color_picker_overlay('overlay_color', $block_id, $overlay_color) ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('bg_color') ?>">
				<?php _e( 'Background Color (Ex : #fafafa;)', 'aakanksha');?>
                <?php echo as_field_color_picker('bg_color', $block_id, $bg_color) ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('position') ?>">
				<?php _e( 'Background position', 'aakanksha');?>
				<?php echo as_field_input('position', $block_id, $position, $size = 'small') ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('repeat') ?>">
				<?php _e( 'Background repeat ?', 'aakanksha');?>
				<?php echo as_field_select('repeat', $block_id, $bg_repeat, $repeat) ?>
			</label>
		</p>
        <p class="description">
			<label for="<?php echo $this->get_field_id('parallax') ?>">
				<?php _e( 'Background parallax ?', 'aakanksha');?>
				<?php echo as_field_checkbox('parallax', $block_id, $parallax) ?>
			</label>
		</p>
        <p class="description">
        ...
        <p class="description">
			<label for="<?php echo $this->get_field_id('menu_id') ?>">
				<?php _e( 'Select the block’s ID to link with the Menu header.', 'aakanksha');?>
				<?php echo as_field_select('menu_id', $block_id, $menus_arr, $menu_id) ?>
			</label>
		</p>	
		<?php
	}
	...
	function block( $instance ) {
        ...
        $overlay_color  = ( ! empty ( $overlay_color ) ) ? 'background-image: linear-gradient( ' . $overlay_color .',' . $overlay_color .') ,url('. $image .') ;background-image:  -webkit-linear-gradient( ' . $overlay_color .',' . $overlay_color .'), url('. $image .');' : '';
		...
		$style = ( 
			...
			! empty( $repeat ) ||
			! empty( $parallax ) ) ? 
				sprintf( '%s %s %s %s %s %s', $image, $overlay_color, $bg_color, $position, $repeat, $parallax ) : '';
		$css_parallax = '';
		if ( ! empty( $style ) ) {			
		...
		echo '<section><div id="'.$menu_id.'" class=" template-wrap cf'.$white_color.'" '.$css_wrapper.' >
...
as_register_block( 'as_Open_Container_Block' );
