...
// TEXT BLOCK
if( ! class_exists( 'as_Text_Block' ) ) :

class as_Text_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT Text', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('as_Text_Block', $block_options);
	}
 	...
 	function form($instance) {
        ...
        <p class="description">
            ...
            <label for="<?php echo $this->get_field_id('font_size') ?>">
                <?php _e('Font-size (Ex : 18px)', 'aakanksha'); ?>
                <?php echo as_field_input('font_size', $block_id, $font_size, 'min', 'number') ?> px
            </label>&nbsp;&nbsp; - &nbsp;
        </p>
        <p class="description">
        ...
        <p class="description">
			<label for="<?php echo $this->get_field_id('text') ?>">
				<?php _e('Content', 'aakanksha'); ?>
				<?php echo as_field_textarea('text', $block_id, $text, $size = 'full') ?>
			</label>
		</p>			
		<?php
	}
...
as_register_block( 'as_Text_Block' );
endif;