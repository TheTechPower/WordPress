...
// BUTTON BLOCK
if( ! class_exists( 'as_Button_Block' ) ) :

class as_Button_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT Button', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('as_Button_Block', $block_options);
	}
 	...
 	function form($instance) {
        ...
        <p class="description">
            <label for="<?php echo $this->get_field_id('duration') ?>">
                <?php _e('Duration for Animation (Ex : 900ms)', 'aakanksha'); ?>
        ...
        <p class="description">
            <label for="<?php echo $this->get_field_id('align') ?>">
                <?php _e('Button align', 'aakanksha'); ?><br/>
                <?php echo as_field_select('align', $block_id, $button_align, $align) ?>
            </label>
        </p>  
        <div class="sortable-body">
        	<p class="description">
                <label for="<?php echo $this->get_field_id('target') ?>">
                    <?php _e('Target', 'aakanksha'); ?><br/>
                    <?php echo as_field_select('target', $block_id, $button_target , $target) ?>
                </label>
            </p>
        	<p class="description">
                <label for="<?php echo $this->get_field_id('link_text') ?>">
                    <?php _e('Text', 'aakanksha'); ?>
                    <?php echo as_field_input('link_text', $block_id, $link_text, $size = 'full') ?>
            	</label>
            </p>    
            <p class="description">
                <label for="<?php echo $this->get_field_id('link') ?>">
                    <?php _e('Link to', 'aakanksha'); ?>
                    <?php echo as_field_input('link', $block_id, $link, $size = 'full') ?>
                </label>
            </p>
        </div>      	
...
as_register_block( 'as_Button_Block' );
endif;