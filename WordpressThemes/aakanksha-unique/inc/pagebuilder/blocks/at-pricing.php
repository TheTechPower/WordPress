...
// PRICING BLOCK
if(!class_exists('as_Pricing_Block')) :

class as_pricing_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT Pricing', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the widget
		parent::__construct('as_Pricing_Block', $block_options);
		
		//add ajax functions
		add_action('wp_ajax_as_block_pricing_add_new', array($this, 'add_pricing'));
	...
	function form($instance) {
		$defaults = array(
			'pricings' => array(
				1 => array(
					'title' 		=> __('Basic <span>$9</span>/month', 'aakanksha'),
					'left'  		=> '0',
					'right'  		=> '0',
					'option_1'  	=> '3G Space',
		...
		<div class="description cf">
			...
			<p></p>
			<a href="#" rel="pricing" class="as-sortable-add-new button">Add New</a>
			<p></p>
		</div>
		<?php
	}
			...
			<div class="sortable-body">
            	<p class="pricing-desc description">
                    <label for="<?php echo $this->get_field_id('pricings') ?>-<?php echo $count ?>-left">
						<?php _e('Margin Left (default: 0 px)', 'aakanksha' ); ?>
					...
					<label for="<?php echo $this->get_field_id('pricings') ?>-<?php echo $count ?>-right">
						<?php _e('Margin Right (default: 0 px)', 'aakanksha' ); ?>
            	...
            	<p class="pricing-desc description">
                      <label for="<?php echo $this->get_field_id('pricings') ?>-<?php echo $count ?>-type_pricing">
                          <?php _e('Type pricing style ?','aakanksha' ); ?>
                          <?php echo as_field_select_in($this->get_field_id('pricings').'-'.$count.'-type_pricing', $this->get_field_name('pricings').'['.$count.'][type_pricing]', $pricing_main, $pricing['type_pricing']) ?>
                      </label>
                </p>
				<p class="pricing-desc description">
                ...
                <p class="pricing-desc description">
                    <label for="<?php echo $this->get_field_id('pricings') ?>-<?php echo $count ?>-content_link">
                        <?php _e('Text button (ex: "sign up")','aakanksha' ); ?><br/>
                ...
                <p class="pricing-desc description">
                      <label for="<?php echo $this->get_field_id('pricings') ?>-<?php echo $count ?>-animation">
                          ...
                          <?php echo as_field_select_in($this->get_field_id('pricings').'-'.$count.'-animation', $this->get_field_name('pricings').'['.$count.'][animation]', $include_animation, $pricing['animation']) ?>
                      </label>
                </p>
                <p class="pricing-desc description">
				...
				<p class="pricing-desc description"><a href="#" class="sortable-delete"><?php _e('Delete','aakanksha' ); ?></a></p>
			</div>
	...
	function block($instance) {
		...
		foreach( $pricings as $pricing ){
			...
			if($pricing['type_pricing'] != ''){
				 $type ='main-pricing';
				 $porpular = __('<span class="popular">POPULAR</span>', 'aakanksha');
			}
	...
	function after_block($instance) {
 		extract($instance);
 		echo '</div><!-- END at-PRICING-BLOCK -->';
	}
 	
}

as_register_block( 'as_pricing_Block' );
