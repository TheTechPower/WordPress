...
// TIMELINE BLOCK
if(!class_exists('as_Timeline_Block')) :

class as_Timeline_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT About', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the widget
		parent::__construct('as_Timeline_Block', $block_options);
		
		//add ajax functions
		add_action('wp_ajax_as_block_timeline_add_new', array($this, 'add_timeline'));
	...
	function form($instance) {
		...
		<div class="description cf">
			...
			<p></p>
			<a href="#" rel="timeline" class="as-sortable-add-new button">Add New</a>
			<p></p>
		</div>
		<?php
	}
			...
			<div class="sortable-body">
				...
				<p class="timeline-desc description">
                      <label for="<?php echo $this->get_field_id('timelines') ?>-<?php echo $count ?>-animation">
                          Animation style
                          <?php echo as_field_select_in($this->get_field_id('timelines').'-'.$count.'-animation', $this->get_field_name('timelines').'['.$count.'][animation]', $include_animation, $timeline['animation']) ?>
                      </label>
                </p>
                <p class="timeline-desc description">
...
as_register_block( 'as_Timeline_Block' );
