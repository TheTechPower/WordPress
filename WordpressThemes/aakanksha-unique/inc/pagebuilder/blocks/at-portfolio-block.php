...
// CLEAR BLOCK
if( ! class_exists( 'as_Portfolio_Block' ) ) :

class as_Portfolio_Block extends as_Block {

	function __construct() {
		$block_options = array(
			'name' => __( 'AT Portfolio', 'aakanksha'),
			'size' => 'span12',
		);
		
		//create the block
		parent::__construct('as_Portfolio_Block', $block_options);
	}
 	...
 	function form($instance) {
		...
		<p class="description">
			<label for="<?php echo $this->get_field_id('p_number') ?>">
				<?php _e('Quantity of porfolio items. ( * Note : with the value \'-1\', all item will be displayed ! )','aakanksha' ); ?>
                <?php echo as_field_input('p_number', $block_id, $p_number, 'min', 'number') ?>
			</label>
		</p>	
        <p class="description">
			<label for="<?php echo $this->get_field_id('hide_filter') ?>">
				<?php _e('Hide filter ?','aakanksha' ); ?>
				<?php echo as_field_checkbox('hide_filter', $block_id, $hide_filter) ?>
			</label>
		</p>	
		<?php
	}
	...
	function block($instance) {
					...
					foreach ($terms as $term) {
						...
						<li>
							<a href="#" data-filter=".<?php echo $term->slug ?>">
								<span class="icon-categories"><?php echo as_get_tax_icon($term->term_id) ?></span>
                            	<?php echo $term->name ?>
                            </a>
						</li>
						<?php
		...
		?>	
			<div class="item isotope-item <?php echo $class ?>">
				...
				<?php the_post_thumbnail( 'portfolio-thumb', array('class' => 'at-portfolio-thumbnail img-responsive') ); ?>
					...
					<div class="container">
						<div class="row">
							<div class="port-control col-md-12">
								<a class="prev" href="#"><span class="arrow-port left"></span>&nbsp;&nbsp;'.__('Prev','aakanksha').'</a> 
								<a href="#" class="close-port"><i class="fa fa-times"></i></a> 
								<a href="#" class="next">'.__('Next','aakanksha').'&nbsp;&nbsp;<span class="arrow-port right"></span></a>
							</div>
						</div>
 	...
 	function before_block($instance) {
		extract($instance);
		echo '<div id="as-block-'.$template_id.'-'.$number.'" class="as-block as-block-'.$id_base.'">';
	...
	function after_block($instance) {
 		extract($instance);
 		echo 	'</div>';
 		echo '</div><!-- END at-PORTFOLIO-BLOCK-->';
	}

}

as_register_block( 'as_Portfolio_Block' );
endif;