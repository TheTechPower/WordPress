...
// Text Block
function as_block_text($block) {
	extract( $block, EXTR_OVERWRITE );
	
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
...
//slogan
function as_block_slogan($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
...
// Slider
function as_block_slider($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
...
// Google Map
function as_block_googlemap($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
...
// Portfolio
function as_block_portfolio($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks['.$block_id.']';
	...
	<p class="description">
		<label for="">
			...
			<?php echo aspb_select($columns_options, $columns, $block_id, 'columns'); ?>
		</label>
	</p>
	<?php
}

function as_block_featured_portfolio($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
...
function as_block_widgets($block) {
	extract( $block, EXTR_OVERWRITE );
	$block_id = 'as_block_' . $number;
	$block_saving_id = 'as_blocks[as_block_'.$number.']';
	...
	<p class="description half last">
		<label for="">
			...
			<?php echo aspb_select($sidebar_options, $sidebar, $block_id, 'sidebar'); ?>
		</label>
	</p>
	<?php
...
function as_block_column($block) {
	echo '<p class="empty-column">',
	__('Drag block items into this box', 'aakanksha'),
	'</p>';
	echo '<ul class="blocks column-blocks cf"></ul>';
}

function as_block_clear($block) {
 ...
 *
 * Also see the aspb_config on adding extra js
 */
function as_ajax_slider_handler() {

}

function as_ajax_slider_display() {
