...
$as_registered_blocks = array();

if(!class_exists('as_Block')) {
	class as_Block {
	 	...
	 	/* PHP4 constructor */
	 	/*
		function as_Block($id_base = false, $block_options = array()) {
	 		as_Block::__construct($id_base, $block_options);
	 	}
	 	*/
	 	/* PHP5 constructor */
	 	...
	 	function block($instance) {
	 		extract($instance);
	 		echo __('function as_Block::block should not be accessed directly. Output generated by the ', 'aakanksha') . strtoupper($id_base). ' Class';
	 	}
	 	...
	 	function form($instance) {
	 		echo '<p class="no-options-block">' . __('There are no options for this block.', 'aakanksha') . '</p>';
	 		return 'noform';
	 	}
	 	...
	 	function form_callback($instance = array()) {
	 		...
	 		$this->block_id = 'as_block_' . $instance['number'];
	 	...
	 	function before_block($instance) {
	 		extract($instance);
	 		$column_class = $first ? 'as-first' : '';
	 		echo '<div id="as-block-'.$template_id.'-'.$number.'" class="as-block as-block-'.$id_base.' '.$size.'">';
			if($id_base == 'as_column_block') echo '<div class="row">';
	 	}
	 	
	 	/* block footer */
	 	function after_block($instance) {
	 		extract($instance);
			if($id_base == 'as_column_block') echo '</div>';
	 		echo '</div>';	
	 	}
	 	...
	 	function get_field_name($field) {
	 		$field_name = isset($this->block_id) ? 'as_blocks[' . $this->block_id. '][' . $field . ']': '';
	 		return $field_name;
	 	}
