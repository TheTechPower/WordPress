...
if(!class_exists('as_Page_Builder')) {
	class as_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Layout Builder', 'aakanksha');
			$defaults['page_title'] = __('Layout Builder', 'aakanksha');
			$defaults['page_slug'] = __('as-page-builder', 'aakanksha');
		...
		function init() {
			...
			add_action('init', array(&$this, 'add_shrotcode'));
		...
		function add_admin_bar(){
			global $wp_admin_bar;
			$wp_admin_bar->add_menu( array( 'id' => 'as-page-builder', 'parent' => 'appearance', 'title' => 'Layout Builder', 'href' => admin_url('themes.php?page='.$this->args['page_slug']) ) );
		...
		function admin_enqueue() {
			...
			// Register 'em
			wp_register_style( 'aspb-css', $this->url.'assets/css/aspb.css', array(), time(), 'all');
			wp_register_style( 'font-awesome', $this->url.'assets/css/font-awesome.min.css', array(), time(), 'all');
			wp_register_style( 'aspb-blocks-css', $this->url.'assets/css/aspb_blocks.css', array(), time(), 'all');
			wp_register_script('aspb-js', $this->url . 'assets/js/aspb.js', array('jquery'), time(), true);
            wp_register_script('aspb-fields-js', $this->url . 'assets/js/aspb-fields.js', array('jquery'), time(), true);
			wp_register_script('bootstrap-js', $this->url . 'assets/js/bootstrap.min.js', array('jquery'), time(), true);
			
			// Enqueue 'em
			wp_enqueue_style('aspb-css');
			wp_enqueue_style('font-awesome');
			wp_enqueue_style('aspb-blocks-css');
			...
			wp_enqueue_script('aspb-js');
			wp_enqueue_script('aspb-fields-js');
			...
			do_action('as-page-builder-admin-enqueue');
		...
		function view_enqueue() {
			...
			// front-end css
			wp_register_style( 'aspb-view-css', $this->url.'assets/css/aspb-view.css', array(), time(), 'all');
			wp_enqueue_style('aspb-view-css');
				
			// front-end js
			wp_register_script('aspb-view-js', $this->url . 'assets/js/aspb-view.js', array('jquery'), time(), true);
			wp_enqueue_script('aspb-view-js');
			
			//hook to register custom styles/scripts
			do_action('as-page-builder-view-enqueue');
		...
		function register_template_post_type() {
			...
			if(!post_type_exists('template')) {
				...
				$regsiter_post_type( 'template', $template_args);
				
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Page Builder notice: </strong>'. __('The \"template\" post type already exists, possibly added by the theme or other plugins. Please consult with theme author to consult with this issue', 'aakanksha') .'</p></div>';"));
			}
		...
		function display_blocks( $template_id ) {
			...
			if(empty($blocks)) {
				...
				echo __('Drag block items from the left into this area to begin building your template.', 'aakanksha');
				echo '</p>';
				return;
				...
				foreach($blocks as $key => $instance) {
					...
					if(isset($as_registered_blocks[$id_base])) {
						//get the block object
						$block = $as_registered_blocks[$id_base];
		...
		**/
		function update_template($template_id, $blocks, $title) {
			...
			foreach ($blocks as $new_instance) {
				...
				$old_key = isset($new_instance['number']) ? 'as_block_' . $new_instance['number'] : 'as_block_0';
				$new_key = isset($new_instance['number']) ? 'as_block_' . $i : 'as_block_0';
				...
				if(class_exists($id_base)) {
					...
					//sanitize instance with as_Block::update()
			...
			//update transient
			$template_transient = 'as_template_' . $template_id;
		...
		**/
		...
		function preview_template() {
			...
			global $wp_query, $as_page_builder;
				...
				?>
					<div id="main" class="cf">
						<div id="content" class="cf">
							...
							<?php if($this->args['debug'] == true) print_r(as_get_blocks(get_the_ID())) //for debugging ?>
						</div>
					</div>
				<?php
				get_footer();
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				echo '<p class="empty-template">';
				echo __('This template is empty', 'aakanksha');
				echo '</p>';
				
			} else {
				//template wrapper
				echo '<div id="as-template-wrapper-'.$template_id.'" class="as-template-wrapper as_row">';
		 ...
		 * @since 1.0.0
		 */
		function add_shrotcode() {
		
			global $shortcode_tags;
			if ( !array_key_exists( 'template', $shortcode_tags ) ) {
                $add_shrotcode = 'add_' . 'shortcode';   
				$add_shrotcode( 'template', array(&$this, 'do_shortcode') );
			} else {
				add_action('admin_notices', create_function('', "echo '<div id=\"message\" class=\"error\"><p><strong>Page Builder notice: </strong>'. __('The \"[template]\" shortcode already exists, possibly added by the theme or other plugins. Please consult with the theme author to consult with this issue', 'aakanksha') .'</p></div>';"));
			}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . aspb_DIR . '/assets/images/media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'aakanksha' )  . '" />';
					$output = '<a href="#TB_inline?width=640&inlineId=aspb-iframe-container" class="thickbox" title="' . esc_attr__( 'Add Page Template', 'aakanksha' )  . '">' . $img . '</a>';
				} else {
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . aspb_DIR . '/assets/images/media-button.png ); margin-top: -1px;"></span>';
					$output = '<a href="#TB_inline?width=640&inlineId=aspb-iframe-container" class="thickbox button" title="' . esc_attr__( 'Add Page Template', 'aakanksha' ) . '" style="padding-left: .4em;">' . $img . ' ' . esc_attr__( 'Add Template', 'aakanksha' ) . '</a>';
				}
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				<script type="text/javascript">
					function insertTemplate() {
						var id = jQuery( '#select-aspb-template' ).val();

						/** Alert user if there is no template selected */
						if ( '' == id ) {
							alert("<?php echo esc_js( __( 'Please select your template first!', 'aakanksha' ) ); ?>");
							return;
						}
				...
				<div id="aspb-iframe-container" style="display: none;">
					<div class="wrap" style="padding: 1em">

						<?php do_action( 'aspb_before_iframe_display', $templates ); ?>	

						<?php
						/** If there is no template created yet */
						if ( empty( $templates ) ) {
							echo sprintf( __( 'You don\'t have any template yet. Let\'s %s create %s one!', 'aakanksha' ), '<a href="' .admin_url().'themes.php?page=as-page-builder">', '</a>' );
							return;
						}
						?>						

						<h3><?php _e( 'Choose Your Page Template', 'aakanksha' ); ?></h3><br />
						<select id="select-aspb-template" style="clear: both; min-width:200px; display: inline-block; margin-right: 3em;">
						<?php
						...
						<input type="button" id="aspb-insert-template" class="button-primary" value="<?php echo esc_attr__( 'Insert Template', 'aakanksha' ); ?>" onclick="insertTemplate();" />
						<a id="aspb-cancel-template" class="button-secondary" onclick="tb_remove();" title="<?php echo esc_attr__( 'Cancel', 'aakanksha' ); ?>"><?php echo esc_attr__( 'Cancel', 'aakanksha' ); ?></a>

						<?php do_action( 'aspb_after_iframe_display', $templates ); ?>
		...
		function contextual_help() {
			...
			$contextual_helps = apply_filters('aspb_contextual_helps', array());
			
			if($screen->id == $this->page) {
				// Help tab sidebar
                /**$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:','aakanksha') . '</strong></p>' .
					'<p>' . __('<a href="" target="_blank">Documentation</a>') . '</p>' .
					'<p>' . __('<a href="" target="_blank">Changelog</a>') . '</p>'
				);**/
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview','aakanksha'),
		...
		function builder_page_show(){
			...
			require_once(aspb_PATH . 'view/view-builder-page.php');
