...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'aakanksha' ),
                'menu_title'                     => __( 'Install Plugins', 'aakanksha' ),
                'installing'                     => __( 'Installing Plugin: %s', 'aakanksha' ),
                'oops'                           => __( 'Something went wrong.', 'aakanksha' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'aakanksha' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'aakanksha' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'aakanksha' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'aakanksha' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'aakanksha' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'aakanksha' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'aakanksha' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'aakanksha' ),
                'install_link'                   => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'aakanksha' ),
                'activate_link'                  => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'aakanksha' ),
                'return'                         => __( 'Return to Required Plugins Installer', 'aakanksha' ),
                'dashboard'                      => __( 'Return to the dashboard', 'aakanksha' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'aakanksha' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'aakanksha' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'aakanksha' ),
                'dismiss'                        => __( 'Dismiss this notice', 'aakanksha' ),
            );
        ...
        public function install_plugins_page() {
            ...
            <div class="tgmpa wrap">
                ...
                <?php if ( version_compare( $this->wp_version, '3.8', '<' ) ) {
                    $srceen_icon =  'screen_' . 'icon';
                    $srceen_icon( apply_filters( 'tgmpa_default_screen_icon', 'themes' ) );
                } ?>
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'aakanksha' ) . '">' . __( 'Return to the Dashboard', 'aakanksha' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'aakanksha' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'aakanksha' ) . '</a>'  : '';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'aakanksha' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'aakanksha' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'aakanksha' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'aakanksha' ) : __( 'Recommended', 'aakanksha' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'aakanksha' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'aakanksha' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'aakanksha' ) . ' %2$s">' . __( 'Install', 'aakanksha' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'aakanksha' ) . ' %2$s">' . __( 'Activate', 'aakanksha' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'aakanksha' ), admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'aakanksha' ),
                'source' => __( 'Source', 'aakanksha' ),
                'type'   => __( 'Type', 'aakanksha' ),
                'status' => __( 'Status', 'aakanksha' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'aakanksha-bulk-install'  => __( 'Install', 'aakanksha' ),
                'aakanksha-bulk-activate' => __( 'Activate', 'aakanksha' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-install' === $this->current_action() ) {
                ...
                if ( empty( $plugin_installs ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be installed at this time.', 'aakanksha' ) . '</p></div>';
                    return false;
                }
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( empty( $plugins_to_activate ) ) {
                    echo '<div id="message" class="error"><p>' . __( 'No plugins are available to be activated at this time.', 'aakanksha' ) . '</p></div>';
                    return false;
                }
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'aakanksha' ), $imploded );
                }
...
/*function tgmpa_load_bulk_installer() {
 ...
 /*               public function bulk_install( $packages ) {
   ...
   /*             public function install_strings() {
                    ...
                    $this->strings['no_package']          = __( 'Install package not available.', 'aakanksha' );
                    $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'aakanksha' );
                    $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'aakanksha' );
                    $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'aakanksha' );
                    $this->strings['process_failed']      = __( 'Plugin install failed.', 'aakanksha' );
                    $this->strings['process_success']     = __( 'Plugin installed successfully.', 'aakanksha' );
 ...
 /*               public function activate_strings() {
                    ...
                    $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'aakanksha' );
                    $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'aakanksha' );
 ...
 /*               public function plugin_info() {
   ...
   /*         class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
    ...
    /*            public function add_strings() {
                    ...
                    if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'aakanksha' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'aakanksha' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'aakanksha' ) . '</span><span class="hidden">' . __( 'Hide Details', 'aakanksha' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'aakanksha' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'aakanksha' );
                    }
                    // Default installation strings.
                    else {
                        $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'aakanksha' );
                        $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'aakanksha' );
                        $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'aakanksha' );
                        $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'aakanksha' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'aakanksha' ) . '</span><span class="hidden">' . __( 'Hide Details', 'aakanksha' ) . '</span>.</a>';
                        $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'aakanksha' );
                        $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'aakanksha' );
                    }
   ...
   /*             public function bulk_footer() {
                    ...
                    if ( empty( $complete ) ) {
                        echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . admin_url() . '" title="' . __( 'Return to the Dashboard', 'aakanksha' ) . '">' . __( 'Return to the Dashboard', 'aakanksha' ) . '</a>' ) . '</p>';
                        echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                    }
